import React, { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { FiPlus, FiArrowRight } from "react-icons/fi";
import { Map, TileLayer, Marker, Popup } from "react-leaflet";

import mapMarkerImg from "../images/map-maker.svg";

import "../styles/pages/orphaneges-map.css";
import happyMapIcon from "../utils/mapIcon";
import api from "../services/api";
import Orphanage from "./Orphanage";

interface Orphanage{
  id: number;
  latitude: number;
  longitude: number;
  name: string;
}

function OrphanagesMap() {
  //Inciar com um array vazio
  //Lista de orfanatos e uma função para atualizar o array
  const [orphanages, setOrphanages] = useState<Orphanage[]>([]);//Especifica o tipo de dados que o state vai ter

  //Executar assim que o componente carregar
  //Qual ação eu quero executar, quando eu quero executar
  useEffect(() => {
    api.get("orphanages").then((response) => {
      //Setando novos valores
      setOrphanages(response.data);
    });
  }, []);

  return (
    <div id="page-map">
      <aside>
        <header>
          <img src={mapMarkerImg} alt="Happy" />

          <h2>Escolha um orfanato no mapa</h2>
          <p>Muitas crianças estão esperando a sua visita :)</p>
        </header>

        <footer>
          <strong>São Paulo</strong>
          <span>Pirapozinho</span>
        </footer>
      </aside>

      <Map
        center={[-22.279686, -51.5098118]}
        zoom={15}
        style={{ width: "100%", height: "100%" }}
      >
        <TileLayer
          url={`https://api.mapbox.com/styles/v1/mapbox/light-v10/tiles/256/{z}/{x}/{y}@2x?access_token=${process.env.REACT_APP_MAPBOX_TOKEN}`}
        />

        {orphanages.map(orphanage => {
          return (
            <Marker 
              icon={happyMapIcon}
              position={[orphanage.latitude, orphanage.longitude]}
              key={orphanage.id}
            >
              <Popup
                closeButton={false}
                minWidth={240}
                maxHeight={240}
                className="map-popup"
              >
                {orphanage.name}
                <Link to={`/orphanages/${orphanage.id}`}>
                  <FiArrowRight size={20} color="#fff" />
                </Link>
              </Popup>
            </Marker>
          );
        })}
      </Map>

      <Link to="/orphanages/create" className="create-orphanage">
        <FiPlus size="32" color="#FFF" />
      </Link>
    </div>
  );
}

export default OrphanagesMap;
